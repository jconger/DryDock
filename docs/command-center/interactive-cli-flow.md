...
Status: Options 1-7 are implemented. Watch/scheduler/service settings are stored via repo config overrides; service control remains planned.
...
flowchart TD
  ...
  E -->|7| S[Prompt owner/repo/config JSON]
  S --> T[Set repo config overrides]
  T --> Z
```
